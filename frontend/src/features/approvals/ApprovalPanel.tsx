...
function ApprovalCard({ approval }: { approval: { id: string; tool_name: string; action: string; reason: string; risk: string } }) {
  const queryClient = useQueryClient()
  const [permissionType, setPermissionType] = useState('once')

  const approveMutation = useMutation({
    mutationFn: (params: { id: string; type: string }) => approveApproval(params.id, params.type),
  ...
  return (
    <div className="bg-surface-300/80 border border-white/5 rounded-lg p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-xs font-medium">{approval.tool_name}</span>
      ...
      <p className="text-[11px] text-gray-400">{approval.reason || approval.action}</p>
      ...
      <div className="flex gap-2">
        <button
          onClick={() => approveMutation.mutate({ id: approval.id, type: permissionType })}
